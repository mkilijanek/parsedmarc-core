...
- `get_dmarc_reports_from_mailbox()` gained an optional keyword-only `save_callback` parameter, invoked once per fetched batch with that batch's newly parsed reports, after parsing but before the batch's messages are deleted or moved out of `reports_folder`. Returning `False` leaves the batch's messages in place for retry and skips the aggregate-report dedup-cache update for that batch; raising also leaves messages untouched and propagates; any other return value commits the batch exactly as before. `save_callback=None` (the default) preserves the prior behavior for any batch that completes without an unhandled exception; a mid-batch crash now leaves `SEEN_AGGREGATE_REPORT_IDS` unmodified for that batch instead of partially populated, which is a related, strictly safer change.
- `watch_inbox()`'s `callback` parameter is now passed through as `save_callback`, so it runs once per batch *before* that batch is archived/deleted rather than afterward, and may return `False` to defer a batch for retry on the next check.
- **`save_callback` can now return a new `SaveOutcome.QUARANTINE` value to stop retrying a batch that will never save** ([domainaware/parsedmarc#823](https://github.com/domainaware/parsedmarc/issues/823) review). Without a cap, a *persistent* sink outage in `--watch` mode meant a batch that failed to save was resent to every configured output on every poll cycle forever — including the non-deduplicating destinations above — turning one outage into an unbounded duplicate backlog once the sink recovered. Returning `SaveOutcome.QUARANTINE` from `save_callback` moves the batch's messages to `<archive_folder>/Quarantine` (created on demand — it is deliberately not part of the existing eager `create_folders` step, since watch mode always calls with `create_folders=False`) instead of leaving them in `reports_folder`, and — like a `False` return — leaves the aggregate-report dedup cache unmodified, so moving the messages back to `reports_folder` later reprocesses them once the underlying issue is fixed. The CLI's `mailbox_save_callback` adapter now tracks a consecutive-failure counter and returns `SaveOutcome.QUARANTINE` once it reaches a hardcoded `MAX_FAILED_SAVE_ATTEMPTS = 10` (no new config option — see `AGENTS.md`'s "adding a config option is a commitment" policy; the threshold is process-local, so it only matters for a long-running `--watch` process, since a single-shot/cron run gets one attempt per process and never quarantines), logging a new default-visible `ERROR`-level line both when a batch is quarantined and when the mailbox folder needed for quarantine can't be created.
- Single-shot CLI runs that configure both file/mbox inputs and a mailbox connection in the same run now emit output (the stdout JSON print, JSON/CSV file appends, and sink writes) in two passes instead of one: mailbox-derived reports are saved via the new `save_callback` before the mailbox connection returns, and file/mbox-derived reports are saved in a second pass. `append_json`/`append_csv` are already append-safe, and watch mode already calls its callback repeatedly, so this is a change in frequency for the single-shot case, not a new code path.
- **The periodic summary email can now be sent via Microsoft Graph** (fork [#19](https://github.com/mkilijanek/parsedmarc-core/issues/19), tracking upstream [domainaware/parsedmarc#472](https://github.com/domainaware/parsedmarc/issues/472)). Previously the summary email required `[smtp] host`, forcing M365 tenants that block legacy SMTP AUTH to stand up a separate SMTP relay just to send from the same mailbox they already read reports from. When `[smtp] host` is omitted and `[msgraph]` is configured, the summary is now sent through the same already-authenticated Graph mailbox connection (`/users/{mailbox}/sendMail`), saved to Sent Items. SMTP is preferred whenever `[smtp] host` is set, with no fallback to Graph on SMTP failure. `[smtp] host`/`user`/`password`/`from` are now conditionally required — only when `host` is present; `to` keeps its existing requirement either way. A new public `email_results_via_msgraph()` function shares its subject/message/zip-building logic with the existing `email_results()` via an extracted `_build_report_email_content()` helper, so both transports stay in lockstep. Note `[smtp] from` has no effect on the Graph path — the message's `From` is always the `[msgraph]` mailbox — and sending requires the Graph `Mail.Send` permission (`Mail.Send.Shared` for a shared mailbox under delegated auth); see the "Sending the summary email via Microsoft Graph" docs section for the full permission matrix.
- **Microsoft Graph connection/fetch/send failures now log a single clear ERROR line** (fork [#20](https://github.com/mkilijanek/parsedmarc-core/issues/20)) instead of a bare `logger.exception()` that hid the actual Azure/Graph error. The line identifies the mailbox, tenant ID, and auth method, and includes the Graph `request-id`/`client-request-id` when available (from the OData inner error or, failing that, the raw response headers) — details that matter when contacting Microsoft support. The full traceback is still preserved under `--debug`.
- **Refreshed the `[msgraph]` documentation** (fork [#25](https://github.com/mkilijanek/parsedmarc-core/issues/25)): national/sovereign-cloud `graph_url` values with an explicit warning that the Entra ID auth endpoint isn't independently configurable, a minimal example config for every auth method, a reading-permission matrix alongside the existing sending one, an accurate note on the `parsedmarc`-named token cache (no migration needed — it's a deliberate backward-compatibility choice from the 9.11.0 `mailsuite` extraction, not something users have to act on), and a troubleshooting table distinguishing still-live failure modes (admin consent, uninitialized-mailbox folder resolution) from historical ones already fixed at this project's dependency floor (`Event loop is closed`, invalid ISO timestamps).
- **Added a "Setting up Gmail API access" walkthrough** (fork [#34](https://github.com/mkilijanek/parsedmarc-core/issues/34), tracking upstream [domainaware/parsedmarc#399](https://github.com/domainaware/parsedmarc/issues/399)/[#354](https://github.com/domainaware/parsedmarc/issues/354)): Google Cloud project/Gmail API/OAuth consent screen setup, the Desktop-app-vs-Web-application client type distinction (a Web application client authenticates but fails with `redirect_uri_mismatch`), what happens on first vs. subsequent runs, and a separate service-account/domain-wide-delegation path for headless deployments. Verified against the installed `google-auth-oauthlib`/`google-auth` source and Google's own current documentation rather than assumed. Found and reported a real upstream `mailsuite` bug while verifying: `[gmail_api] oauth2_port` is silently ignored (`_get_creds()` passes it under the wrong keyword to `run_local_server()`, so the local OAuth redirect listener always binds `8080` regardless of configuration) — filed as [seanthegeek/mailsuite#53](https://github.com/seanthegeek/mailsuite/pull/53), noted in the docs until a fixed `mailsuite` release is available. Also cross-referenced the Gmail-specific `https://mail.google.com/` scope requirement for `[mailbox] delete` from the `delete` option itself, which previously only surfaced as a runtime log message.

### Bug fixes

- **`--watch` no longer crashes with a raw uncaught traceback on a Microsoft Graph error** (fork [#20](https://github.com/mkilijanek/parsedmarc-core/issues/20)). The continuous-mode loop previously caught only `FileExistsError`/`ParserError`; a Graph auth, API, or transport error during a long-running watch — arguably the most likely real-world failure point, since that's where token/certificate expiry actually surfaces — crashed uncaught. It now gets the same single formatted ERROR line as the other Graph call sites and exits cleanly.
- **Mailbox messages are no longer deleted/archived until output sinks confirm the batch was saved** (fork [#33](https://github.com/mkilijanek/parsedmarc-core/issues/33), tracking upstream [domainaware/parsedmarc#242](https://github.com/domainaware/parsedmarc/issues/242)). Previously, `get_dmarc_reports_from_mailbox()` archived or deleted every successfully-parsed message *before* the CLI had written the parsed reports to Elasticsearch/OpenSearch/Splunk/S3/Kafka/PostgreSQL/etc. If any output sink failed (e.g. an Elasticsearch outage), the report was never persisted anywhere and its source message was already gone from — or moved out of — the mailbox, with no way to recover it. The mailbox is now only mutated once the CLI confirms a batch was actually saved: if *any* output destination fails for a batch, the whole batch's messages stay in `reports_folder` and are retried automatically on the next run/poll — no new config option needed. This is an all-or-nothing rule per batch (archiving on partial success would still permanently lose data for whichever sink failed), and it applies independently of `fail_on_output_error`, which only controls the process exit code. Retrying can result in duplicate delivery to output destinations that don't deduplicate (Kafka, Splunk HEC, Syslog, GELF, Webhook, and `--output` JSON/CSV file appends — `append_json`/`append_csv` unconditionally merge/append every call, so a retried batch produces a duplicate array entry/CSV row); Elasticsearch, OpenSearch, and PostgreSQL already dedupe via `AlreadySaved`, and S3 writes to a deterministic `.../year=YYYY/month=MM/day=DD/{report_id}.json` key, so a retry overwrites the same object instead of duplicating it.
- **A failed JSON/CSV/sample file write no longer crashes the run uncaught.** The `--output` directory writer (`save_output()`, called from `process_reports()`) was the only sink not wrapped in a try/except, so a disk-full or permission error there crashed the process instead of being recorded like every other sink's failures — and, combined with the mailbox-persistence fix above, could otherwise leave mailbox archiving in an inconsistent state relative to what was actually written to disk. It is now caught and recorded as an output error (`"File output"`), so it correctly blocks mailbox archiving like any other sink failure.
- **The emailed summary (`smtp_host`) once again respects `index_prefix_domain_map` filtering of SMTP TLS reports.** The mailbox-persistence change above introduced a second, batch-scoped call to `process_reports()`, so the CLI's shared `parsing_results` dict passed to `email_results()` is built independently of the filtering `process_reports()` applies to the batches it actually saves. The filter is now applied to `parsing_results["smtp_tls_reports"]` as well, so a config combining `index_prefix_domain_map` and `smtp_host` once again excludes unmapped-domain SMTP TLS reports from the emailed summary.
- **An outdated Python patch release no longer masquerades as an invalid report** (fork [#30](https://github.com/mkilijanek/parsedmarc-core/issues/30), tracking upstream [domainaware/parsedmarc#808](https://github.com/domainaware/parsedmarc/issues/808)). The third-party `mail-parser` package unconditionally calls `email.utils.getaddresses([...], strict=True)`, a keyword only supported on Python patch releases `>=3.10.15`, `>=3.11.10`, `>=3.12.6` (the CVE-2023-27043 hardening). On an older patch release within an otherwise-supported minor version, `mailparser.parse_from_string()` raised `TypeError: getaddresses() got an unexpected keyword argument 'strict'`, which parsedmarc's existing broad exception handlers silently turned into a generic "invalid report" error — giving no indication that the report itself was fine and the real cause was an outdated Python patch release. The `TypeError` is now caught right at the `mailparser.parse_from_string()` call sites and re-raised as an `EmailParserError` naming the Python versions required to fix it, guarded on `email.utils.supports_strict_parsing` so unrelated `TypeError`s are untouched and still propagate as before. The real fix belongs upstream in `mail-parser` ([SpamScope/mail-parser#162](https://github.com/SpamScope/mail-parser/pull/162)); this only makes the failure mode legible until that lands.
...
- Added per-version `Programming Language :: Python :: 3.10`–`3.14` (and `3 :: Only`) trove classifiers so the PyPI page advertises the supported Python versions. No change to `requires-python` (`>=3.10`).
