...
"""
    ...
    def testCliMsGraphLoggingNeverLogsSecretValues(
        ...
        """Even at --debug, secret values (certificate_password here) must
        not appear anywhere in parsedmarc's log output — the debug detail
        line reports set/not-set flags instead."""
        mock_graph_connection.return_value = object()
        mock_get_mailbox_reports.return_value = {
            "aggregate_reports": [],
            "failure_reports": [],
